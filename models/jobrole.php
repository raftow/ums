...
class Jobrole extends UmsObject
{
        ...
        public static function loadByMainIndex($id_domain, $jobrole_code, $create_obj_if_not_found = false)
        {
                $obj = new Jobrole();
                if (!$id_domain)
                        throw new AfwRuntimeException('loadByMainIndex : id_domain is mandatory field');
                if (!$jobrole_code)
                        throw new AfwRuntimeException('loadByMainIndex : jobrole_code is mandatory field');
        ...
        public function getDropDownDisplay($lang = 'ar')
        {
                ...
                $domain = $this->decode('id_domain', '', false, $lang);
        ...
        protected function getOtherLinksArray($mode, $genereLog = false, $step = 'all')
        {
                ...
                if ($Main_Page == 'afw_mode_edit.php')
                        $mode_origin = 'edit';
                if ($Main_Page == 'afw_mode_qedit.php')
                        $mode_origin = 'qedit';
                                ...
                                if ($jobGoal->isOk(true)) {
                                        ...
                                        if ($ancetre) {
                                                ...
                                                if ($jar->is_new)
                                                        ...
                                                        $attachedAlready_arr[] = $jar->getDisplay($lang);
                ...
                switch ($attribute) {
                        ...
                        case 'myRoles':
                                ...
                                if ($mainApplication)
                                        ...
                                        $mainApplication_id = 0;
                                if (!$mainApplication_id)
                                        $mainApplication_id = 0;
        ...
        public function findGoalWithCodeEndsWith($suffix, $atable_id = 0)
        {
                ...
                foreach ($goalListMatrix as $goalList) {
                        foreach ($goalList as $goalItem) {
                                ...
                                if (AfwStringHelper::stringStartsWith($goalItemCode, $suffix))
                                        return $goalItem;
                        }
                }
        ...
        public function genereMainGoal($lang = 'ar')
        {
                ...
                if (!$goalObj) {
                        if (!$mainApplication or ($mainApplication->isEmpty()))
                                return array('', 'mainApplication is needed to generate the main goal');
                        $system_id = $mainApplication->getVal('id_system');
                        $module_id = $mainApplication->getId();

                        if (!$system_id)
                                return array('', "the system of mainApplication $mainApplication is not defined");
                ...
                if (!$goalObj->isEmpty())
                        $info .= "common goal $action for this jobrole :  $goalObj<br>";
                else
                        $err .= "common goal not $action for this jobrole :  $goalObj<br>";
