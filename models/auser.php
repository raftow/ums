...
class Auser extends UmsObject implements AfwFrontEndUser
{
        ...
        public static function loadByEmail($email, $create_obj_if_not_found = false)
        {
                $obj = new Auser();
                if (!$email)
                        throw new AfwRuntimeException('loadByMainIndex : email is mandatory field');
        ...
        public static function loadByUsername($username)
        {
                $obj = new Auser();
                if (!$username)
                        throw new AfwRuntimeException('loadByUsername : username is mandatory attribute');
        ...
        public function i_belong_to_one_of_ugroups($authorized_ugroups, $dataObj)
        {
                ...
                foreach ($authorized_ugroups as $ugroup) {
                        ...
                        if ($ugroup->isMember($this, $dataObj))
                                return true;
                }
        ...
        public final function isAdmin()
        {
                ...
                if ($this->getVal('hierarchy_level_enum') <= $admin_max_level)
                        return true;
                $arr_admin = [];
                if (class_exists('AfwSession')) {
        ...
        public final function isSupervisor()
        {
                ...
                if ($this->getVal('hierarchy_level_enum') <= $supervisor_max_level)
                        return true;
                $arr_supervisor = [];
                if (class_exists('AfwSession')) {
        ...
        public function hasModule($mod)
        {
                ...
                $mau_found = null;
                if ($my_module_id)
                        $mau_found = $this->findMyModule($my_module_id);
                if ($mau_found)
                        $report_arr[] = 'mau_found => count = ' . count($mau_found);
                else
                        $report_arr[] = 'mau_found empty ';
        ...
        public function iCanDoOperationOnObjClass($myObj, $operation_sql)
        {
                ...
                if ($operation_sql == 'update')
                        $operation_sql = 'edit';
                elseif ($operation_sql == 'view')
                        $operation_sql = 'display';
                ...
                if (($operation_sql == 'display') and $myObj->public_display)
                        return true;
                if (($operation_sql == 'display') and $myObj->canBePublicDisplayed()) {
                        return true;
                }
                if ($operation_sql == 'display')
                        AfwSession::contextLog("$myObj_displ is not public display and can 'not' BePublicDisplayed()", 'iCanDo');

                if (($operation_sql == 'display') and $myObj->canBeSpeciallyDisplayedBy($this)) {
                        return true;
                }
                if ($operation_sql == 'display')
                        AfwSession::contextLog("$myObj_displ can 'not' BeSpeciallyDisplayedBy($this)", 'iCanDo');
        ...
        public function iCanDoBF($bfId, $moduleCode)
        {
                ...
                if (!empty($can))
                        AfwSession::contextLog("iCanDoBF : $cache_user_can_bf_code found in cache = $can, stop !", 'iCanDo');
                if ($can == 'Y')
                        return 'selon the cache I can do';
                if ($can == 'N')
                        return false;
                ...
                $_isPublicLevel = ($bf_info["level"] == 9999);
                if ($_isPublicLevel) {
                ...
                if (!$mau_found_roles_ids)
                        $mau_found_roles_ids_display = 'no-roles';
                else
                        $mau_found_roles_ids_display = $mau_found_roles_ids;
        ...
        public function getContextValue($currmod, $attribute)
        {
                ...
                if (($currmod == 'ria') and is_object($objme) and ($objme->getId() == $this->getId()) and ($objme->contextSchoolId > 0)) {
                        if ($attribute == 'group_num')
                                return $objme->contextGroupNum;
                        if ($attribute == 'school_id')
                                return $objme->contextSchoolId;
                        if ($attribute == 'year')
                                return $objme->contextYear;
                }
        ...
        public function getMyDepartmentBranchIds($module = '')
        {
                ...
                if ($module)
                        throw new AfwRuntimeException("user has only one organization => module param is obsolete but has value = $module");
                ...
                if ($empl and (!$empl->isEmpty())) {
                        ...
                        if ($empl_dep > 0)
                                return $empl->get('id_sh_dep')->getBranchListOfIDs();
                }
                return 0;
        }
        ...
        public function getMyDepartmentName($lang = 'ar')
        {
                ...
                if ($empl and (!$empl->isEmpty())) {
                        ...
                        if ($depObj)
                                return $depObj->getDisplay($lang);
                        $divObj = $empl->het('id_sh_div');
                        if ($divObj)
                                return $divObj->getDisplay($lang);
                } else {
                        // rafik : @todo to be better studied
                        if (AfwSession::config('student_user_enabled', false)) {
                                $stdnt = $this->getStudent();
                ...
                if ($empl and (!$empl->isEmpty())) {
                        ...
                        if ($stdnt) {
                                ...
                                if ($school) {
                                        ...
                                        if ($dep)
                                                $myOrganizationId = $dep->getVal('id_sh_org');
                                        else
                                                $myOrganizationId = 0;
                                } else
                                        $myOrganizationId = 0;
        ...
        {
                if (!$password_sent_by)
                        $password_sent_by = AfwSession::config('password_sent_by', []);
                $objme = AfwSession::getUserConnected();
                ...
                $firstname_en = str_replace(' ', '', $firstname_en);
                $part1fn = substr($firstname_en, 0, 3);
                $part2fn = substr($firstname_en, 3, 3);
                if (!$username) {
                        $username = $this->getVal('mobile');
                        if (!$username)
                                $username = $this->getVal('idn');
                        if (!$username)
                                $username = $this->getId();
                        $username = 'u' . $username;
                ...
                $sent_by = 'nothing';
                $sent_to = 'nobody';
                $len = AfwSession::config('password_generated_length', 10) - 6;
                if ($len < 3)
                        $len = 3;
                if ($len > 5)
                        $len = 5;
                ...
                if ($objme and $objme->isSuperAdmin())
                        $info_detail = "<!-- username=$username  enc($pwd)=$pwd_enc -->";
                if (!$message_prefix)
                        $message_prefix = $firstname;
                ...
                foreach ($password_sent_by as $password_sent_by_try) {
                        if ($password_sent_by_try == 'sms') {
                                ...
                                if ($simulate_sms_to_mobile)
                                        $sms_mobile = $simulate_sms_to_mobile;
                                else
        ...
        public function afterUpdate($id, $fields_updated, $disableAfterCommitDBEvent = false)
        {
                ...
                if (($objme) and ($this->getId() == $objme->getId())) {
                        ...
                        if ($languageOfUser)
                                AfwSession::setVar('lang', $languageOfUser);
                }
        ...
        protected function importRecord($dataRecord, $orgunit_id, $overwrite_data, $options, $lang, $dont_check_error)
        {
                ...
                if ($idn_correct) {
                        ...
                        if ($overwrite_data or $user->is_new) {
                                if ($genre)
                                        list($val_ok, $val_parsed_or_error) = AfwFormatHelper::parseAttribute($user, 'genre_id', $genre, $lang);
                                else
                                        $val_ok = true;
                                if (!$val_ok)
                                ...
                                if ($nationality)
                                        list($val_ok, $val_parsed_or_error) = AfwFormatHelper::parseAttribute($user, 'country_id', $nationality, $lang);
                                else
                                        $val_ok = true;
                                if (!$val_ok)
                                        $errors[] = $val_parsed_or_error;
                                ...
                                if ($firstname)
                                        list($val_ok, $val_parsed_or_error) = AfwFormatHelper::parseAttribute($user, 'firstname', $firstname, $lang);
                                else
                                        $val_ok = true;
                                if (!$val_ok)
                                        $errors[] = $val_parsed_or_error;
                                ...
                                if ($fatherfirstname)
                                        list($val_ok, $val_parsed_or_error) = AfwFormatHelper::parseAttribute($user, 'f_firstname', $fatherfirstname, $lang);
                                else
                                        $val_ok = true;
                                if (!$val_ok)
                                        $errors[] = $val_parsed_or_error;
                                ...
                                if ($grandfathername)
                                        list($val_ok, $val_parsed_or_error) = AfwFormatHelper::parseAttribute($user, 'g_f_firstname', $grandfathername, $lang);
                                else
                                        $val_ok = true;
                                if (!$val_ok)
                                        $errors[] = $val_parsed_or_error;
                                ...
                                if ($lastname)
                                        list($val_ok, $val_parsed_or_error) = AfwFormatHelper::parseAttribute($user, 'lastname', $lastname, $lang);
                                else
                                        $val_ok = true;
                                if (!$val_ok)
                                        $errors[] = $val_parsed_or_error;
                                ...
                                if ($mobile)
                                        list($val_ok, $val_parsed_or_error) = AfwFormatHelper::parseAttribute($user, 'mobile', $mobile, $lang);
                                else
                                        $val_ok = true;
                                if (!$val_ok)
                                        $errors[] = $val_parsed_or_error;
        ...
        {
                $this_id = $this->getId();
                if ($this_id > self::$MAX_USERS_CRM_START)
                        return true;
                else
                        return false;
        }
        ...
        public function getCustomerId()
        {
                $this_id = $this->getId();
                if ($this_id > self::$MAX_USERS_CRM_START)
                        return ($this_id - self::$MAX_USERS_CRM_START);
        ...
        public function initWithModule($MODULE)
        {
                ...
                if ($MODULE == 'lib')
                        return array(true, 'done');  // throw new AfwRuntimeException("initWithModule($MODULE)???");
        ...
        public function canDisableRO($desc, $module_code)
        {
                ...
                foreach ($found_roles_ids_arr as $found_role_id) {
                        if ($desc["DISABLE-READONLY-$found_role_id"])
                                return true;
                }
